...
public class KingMoveCalculator {
    ...
    public Collection<ChessMove> pieceMoves(ChessBoard board, ChessPosition myPosition) {
        ...
        for (int i = -1; i <= 1; i++) {
            for (int j = -1; j <= 1; j++) {
                if (!(i == 0 && j == 0) && (myPosition.getRow() + i <= 8 && myPosition.getRow() + i >= 1) &&
                    ...
                    if (board.getPiece(new ChessPosition(myPosition.getRow() + i, myPosition.getColumn() + j)) == null) {
                        validKingMoves.add(new ChessMove(myPosition, new ChessPosition(myPosition.getRow() + i,
                                myPosition.getColumn() + j), null));
                    } else if (board.getPiece(new ChessPosition(myPosition.getRow() + i,
