        ...
        for (int[] i : moves) {
            if (myPosition.getRow()    + i[0] <= 8 && myPosition.getRow()    + i[0] >= 1 &&
                ...
                if (board.getPiece(new ChessPosition(myPosition.getRow() + i[0], myPosition.getColumn() + i[1])) == null) {
                    validKnightMoves.add(new ChessMove(myPosition, new ChessPosition(myPosition.getRow() + i[0],
                            myPosition.getColumn() + i[1]), null));
                } else if (board.getPiece(new ChessPosition(myPosition.getRow() + i[0],
                        myPosition.getColumn() + i[1])).getTeamColor() != board.getPiece(myPosition).getTeamColor()) {
                    validKnightMoves.add(new ChessMove(myPosition, new ChessPosition(myPosition.getRow() + i[0],
                            myPosition.getColumn() + i[1]), null));
                }
            }
